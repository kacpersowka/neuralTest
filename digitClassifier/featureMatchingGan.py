    ...
    for z in range(n):
        ...
        for i in range(len(x)):
            ...
            for l in range(len(wd)):
                ...
                dta.append(numpy.array(h))
            hd=dta[-2]
            Lg=((sum(hd)/len(hd))-(sum(hg)/len(hg)))**2 #Loss function for generator
            print('Discriminator score for generated sample: ',da[-1])
            print('Discriminator score for true sample: ',dta[-1])
            print('Generator loss: ',Lg)
            #**Get gradient for generator**
            dLgdhg=numpy.dot(((sum(hd)/len(hd))-(sum(hg)/len(hg))),numpy.array([-2/len(hg) for i in range(len(hg))])) #Generator loss gradient (only need w.r.t hg since training generator)
            dg=dLgdhg #Discriminator gradients, bridges gap from loss to end of generator
...
if __name__=="__main__":
    ...
    ld=[28**2,16,1] #Discriminator layout
    lg=[8,16,28**2] #Generator layout
    #He initialization
